...
def main():
    ...
    check_script_list(src_dir=config["environment"]
                      ["SRCDIR"], fail_on_warn=args.ci)
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control):
    ...
    for i in range(test_count):
        ...
        done_str = "{}/{} - {}{}{}".format(i + 1,
                                           test_count, BOLD[1], test_result.name, BOLD[0])
        if test_result.status == "Passed":
            ...
            if combined_logs_len and os.path.isdir(testdir):
                ...
                print('============\n')
                combined_logs_args = [sys.executable, os.path.join(
...
def check_script_prefixes():
    ...
    good_prefixes_re = re.compile(
        "^(example|feature|interface|mempool|mining|p2p|rpc|wallet|tool|framework_test|qtum)_")
    bad_script_names = [
        script for script in ALL_SCRIPTS if good_prefixes_re.match(script) is None]

    if bad_script_names:
        print("%sERROR:%s %d tests not meeting naming conventions:" %
